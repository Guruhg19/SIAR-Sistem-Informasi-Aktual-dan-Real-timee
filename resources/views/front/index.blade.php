...
<html>
	...
	<body class="font-[Poppins] pb-[72px]">
		...
		<nav id="Category" class="max-w-[1130px] mx-auto flex justify-center items-center gap-4 mt-[30px]">
            ...
            @empty
                <p>Data Category Belum Tersedia</p>
            @endforelse
		</nav>
		<section id="Featured" class="mt-[30px]">
			<div class="main-carousel w-full">
                ...
                @empty
                    <p>Belum tersedia Featured Image</p>
                @endforelse
			</div>
		</section>
		<section id="Up-to-date" class="max-w-[1130px] mx-auto flex flex-col gap-[30px] mt-[70px]">
			...
			<div class="grid grid-cols-3 gap-[30px]">
                ...
                @empty
                    <p>Belum terUpDate Belum Tersedia.</p>
                @endforelse
			</div>
		</section>
		<section id="Best-authors" class="max-w-[1130px] mx-auto flex flex-col gap-[30px] mt-[70px]">
			...
			<div class="grid grid-cols-6 gap-[30px]">
                ...
                @empty
                    <p>Data Author belum tersedia</p>
                @endforelse
			</div>
		</section>
		...
		<section id="Latest-entertainment" class="max-w-[1130px] mx-auto flex flex-col gap-[30px] mt-[70px]">
			...
			<div class="flex justify-between items-center h-fit">
				<div class="featured-news-card relative w-full h-[424px] flex flex-1 rounded-[20px] overflow-hidden">
					<img src="{{Storage::url($entertainment_featured_article->thumbnail)}}" class="thumbnail absolute w-full h-full object-cover" alt="icon" />
					<div class="w-full h-full bg-gradient-to-b from-[rgba(0,0,0,0)] to-[rgba(0,0,0,0.9)] absolute z-10"></div>
					<div class="card-detail w-full flex items-end p-[30px] relative z-20">
						<div class="flex flex-col gap-[10px]">
							<p class="text-white">Featured</p>
							<a href="{{route('front.details', $entertainment_featured_article->slug)}}" class="font-bold text-[30px] leading-[36px] text-white hover:underline transition-all duration-300">{{$entertainment_featured_article->name}}</a>
							<p class="text-white">{{ $entertainment_featured_article->created_at->format('M d, Y') }}</p>
						</div>
					</div>
				</div>
                                ...
                                <div class="rounded-[20px] border border-[#EEF0F7] p-[14px] flex items-center gap-4 hover:ring-2 hover:ring-[#FF6B18] transition-all duration-300">
                                    ...
                                    <div class="flex flex-col justify-center-center gap-[6px]">
                                        <h3 class="font-bold text-lg leading-[27px]">
                                            {{ substr($entertainment_article->name, 0, 50) }}{{ strlen($entertainment_article->name) > 50 ? '...' : '' }}</h3>
                                        <p class="text-sm leading-[21px] text-[#A3A6AE]">{{ $entertainment_article->created_at->format('M d, Y') }}</p>
                                    </div>
                                </div>
                            </a>
                        @empty
                            <p>Article belum tersedia.</p>
                        @endforelse
					</div>
		...
		<section id="Latest-business" class="max-w-[1130px] mx-auto flex flex-col gap-[30px] mt-[70px]">
			...
			<div class="flex justify-between items-center h-fit">
				<div class="featured-news-card relative w-full h-[424px] flex flex-1 rounded-[20px] overflow-hidden">
					<img src="{{Storage::url($business_featured_article->thumbnail)}}" class="thumbnail absolute w-full h-full object-cover" alt="icon" />
					<div class="w-full h-full bg-gradient-to-b from-[rgba(0,0,0,0)] to-[rgba(0,0,0,0.9)] absolute z-10"></div>
					<div class="card-detail w-full flex items-end p-[30px] relative z-20">
						<div class="flex flex-col gap-[10px]">
							<p class="text-white">Featured</p>
							<a href="{{ route('front.details', $business_featured_article->slug) }}" class="font-bold text-[30px] leading-[36px] text-white hover:underline transition-all duration-300">{{ $business_featured_article->name }}</a>
							<p class="text-white">{{ $business_featured_article->created_at->format('M d, Y') }}</p>
						</div>
					</div>
				</div>
				<div class="h-[424px] w-fit px-5 overflow-y-scroll overflow-x-hidden relative custom-scrollbar">
					<div class="w-[455px] flex flex-col gap-5 shrink-0">
						@forelse ($business_articles as $business_article)
                            <a href="{{ route('front.details', $business_article->slug) }}" class="card py-[2px]">
                                <div class="rounded-[20px] border border-[#EEF0F7] p-[14px] flex items-center gap-4 hover:ring-2 hover:ring-[#FF6B18] transition-all duration-300">
                                    <div class="w-[130px] h-[100px] flex shrink-0 rounded-[20px] overflow-hidden">
                                        <img src="{{Storage::url($business_article->thumbnail)}}" class="object-cover w-full h-full" alt="thumbnail" />
                                    </div>
                                    <div class="flex flex-col justify-center-center gap-[6px]">
                                        <h3 class="font-bold text-lg leading-[27px]">{{ $business_article->name }}</h3>
                                        <p class="text-sm leading-[21px] text-[#A3A6AE]">{{ $business_article->created_at->format('M d, Y') }}</p>
                                    </div>
                                </div>
                            </a>
                        @empty
                            <p>Belum ada Article Terkait.</p>
                        @endforelse
					</div>
					<div class="sticky z-10 bottom-0 w-full h-[100px] bg-gradient-to-b from-[rgba(255,255,255,0.19)] to-[rgba(255,255,255,1)]"></div>
				</div>
		...
		<section id="Latest-automotive" class="max-w-[1130px] mx-auto flex flex-col gap-[30px] mt-[70px]">
			...
			<div class="flex justify-between items-center h-fit">
				<div class="featured-news-card relative w-full h-[424px] flex flex-1 rounded-[20px] overflow-hidden">
					<img src="{{Storage::url($automotive_featured_article->thumbnail)}}" class="thumbnail absolute w-full h-full object-cover" alt="icon" />
					<div class="w-full h-full bg-gradient-to-b from-[rgba(0,0,0,0)] to-[rgba(0,0,0,0.9)] absolute z-10"></div>
					<div class="card-detail w-full flex items-end p-[30px] relative z-20">
						<div class="flex flex-col gap-[10px]">
							<p class="text-white">Featured</p>
							<a href="{{ route('front.details', $automotive_featured_article->slug) }}" class="font-bold text-[30px] leading-[36px] text-white hover:underline transition-all duration-300">{{ $automotive_featured_article->name }}</a>
							<p class="text-white">{{$automotive_featured_article->created_at->format('M d, Y')}}</p>
						</div>
					</div>
				</div>
				<div class="h-[424px] w-fit px-5 overflow-y-scroll overflow-x-hidden relative custom-scrollbar">
						...
						@forelse ($automotive_articles as $automotive_article)
                            <a href="{{ route('front.details', $automotive_article->slug) }}" class="card py-[2px]">
                                <div class="rounded-[20px] border border-[#EEF0F7] p-[14px] flex items-center gap-4 hover:ring-2 hover:ring-[#FF6B18] transition-all duration-300">
                                    ...
                                    <div class="flex flex-col justify-center-center gap-[6px]">
                                        <h3 class="font-bold text-lg leading-[27px]">{{ $automotive_article->name }}</h3>
