	...
	<section id="Article-container" class="max-w-[1130px] mx-auto flex gap-20 mt-[50px]">
		...
		<div class="side-bar flex flex-col w-[300px] shrink-0 gap-10">
			...
			<div id="More-from-author" class="flex flex-col gap-4">
                ...
                @forelse ($author_news as $authorNewsItem)
                    <a href="{{ route('front.details', $authorNewsItem->slug) }}" class="card-from-author">
                        <div
                            ...
                            <div class="w-[70px] h-[70px] flex shrink-0 overflow-hidden rounded-2xl">
                                <img src="{{ Storage::url($authorNewsItem->thumbnail) }}" class="object-cover w-full h-full"
                                    alt="thumbnail">
                            </div>
                            <div class="flex flex-col gap-[6px]">
                                <p class="line-clamp-2 font-bold">{{ $authorNewsItem->name }}</p>
                                <p class="text-xs leading-[18px] text-[#A3A6AE]">{{ $authorNewsItem->created_at->format('D m, Y') }} • {{ $authorNewsItem->category->name }}</p>
                            </div>
                        </div>
                    </a>
                @empty
	...
	<section id="Up-to-date" class="w-full flex justify-center mt-[70px] py-[50px] bg-[#F9F9FC]">
		<div class="max-w-[1130px] mx-auto flex flex-col gap-[30px]">
			...
			<div class="grid grid-cols-3 gap-[30px]">
                ...
                <a href="{{ route('front.details', $articleItem->slug) }}" class="card">
                    <div
                        class="flex flex-col gap-4 p-[26px_20px] transition-all duration-300 ring-1 ring-[#EEF0F7] hover:ring-2 hover:ring-[#FF6B18] rounded-[20px] overflow-hidden bg-white">
                        <div class="thumbnail-container h-[200px] relative rounded-[20px] overflow-hidden">
                            <div
                                class="badge absolute left-5 top-5 bottom-auto right-auto flex p-[8px_18px] bg-white rounded-[50px]">
                                <p class="text-xs leading-[18px] font-bold">{{ $articleItem->category->name }}</p>
                            </div>
                            <img src="{{Storage::url($articleItem->thumbnail)}}" alt="thumbnail photo"
                                class="w-full h-full object-cover" />
                        </div>
                        <div class="flex flex-col gap-[6px]">
                            <h3 class="text-lg leading-[27px] font-bold">{{ $articleItem->name }}</h3>
                            <p class="text-sm leading-[21px] text-[#A3A6AE]">{{ $articleItem->created_at->format('M d, Y') }}</p>
                        </div>
                    </div>
                </a>
                @empty
...
@endpush
@push('after-scripts')
	<script src="{{ asset('two-lines-text.js') }}"></script>
@endpush